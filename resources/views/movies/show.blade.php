                    ...
                    <div class="flex items-center">
                        <div class="flex items-center ">                                
                            <form action="{{ route('movies.edit', $movie->id) }}" method="get"> 
                                <button type="submit" class="ml-6 text-lg leading-7 font-semibold shadow">Editar</buttom>
                            </form>  
                            <form action="{{ route('movies.destroy', $movie->id) }}" method="post"> 
                                @csrf                                  
                                @method ('DELETE')
                                <!--<input type="hidden" name="_method" value="DELETE">-->
                                <button type="submit" style="margin:5px;" class="ml-6 text-lg leading-7 font-semibold shadow">Remover</buttom>
                            </form>  
                        </div>
                    </div>
                </div>            
